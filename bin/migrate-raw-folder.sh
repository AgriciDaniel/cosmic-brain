...
#
# Why: Obsidian and the Obsidian Web Clipper hide dot-prefixed folders. The
# canonical source layer used to be `.raw/`; it is now `raw/` so the Clipper
# can target it and so it shows up in the file explorer.
#
# What it does:
...
if [ -d "wiki/sources" ]; then
  ...
  for f in wiki/sources/*.md; do
    ...
    if grep -q 'raw_file: *"\.raw/' "$f"; then
      # Edit in place; only touches the raw_file: line. Portable BSD/GNU sed.
...
echo "Next: in Obsidian Web Clipper, set destination folder to 'raw/' (no dot)."
