...
**Precondition.** Before doing anything, verify the canonical source folder. If `raw/` is missing but a legacy `.raw/` exists, stop and tell the user:

> Your vault still uses the legacy `.raw/` source folder. Run `bash bin/migrate-raw-folder.sh` to rename it to `raw/` (preserves git history, rewrites manifest keys and frontmatter back-pointers). Then re-run ingest.

Do not auto-migrate from within this skill. Do not fall back to `.raw/`.
