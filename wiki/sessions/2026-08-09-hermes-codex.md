...
- Live RAM follow-up: 19.84/39.86 GB used (49.8%) with 20.03 GB free, so physical RAM is not the active inference bottleneck. Largest groups were Node 4.48 GB, Chrome 4.38 GB, Firefox 2.08 GB, and ChatGPT/Codex/Claude 2.61 GB combined. Idle VRAM was 1045 MiB used/4922 MiB free with Ollama unloaded. Opened tracker issue #4 for a user-coordinated GUI restart/closure and before-after measurement rather than terminating active agent/browser work.
- Mapped all Node processes: approximately seven SwarmVault/local-knowledge MCP instances use 1.16 GB; roughly twelve OpenWiki SQLite MCP connections use 2.6 GB across their npx launcher/server pairs; Codex CLI session A owns 1.64 GB, the delegated Claude SSH agent 1.49 GB, the current Codex desktop 0.44 GB, Codex CLI session B 0.43 GB, the Khutba frontend/backend dev stack 0.34 GB on ports 5173/3001, and 9router 0.17 GB on port 20128. All groups were CPU-idle over a five-second sample. Recommended closing unused parent sessions or the Khutba dev stack, not killing individual MCP children.
