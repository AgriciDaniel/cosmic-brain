...
1  Enhanced Compensation Rules - Overview Error! Bookmark not defined.
...
o  Rules  to  report,  summarize  or  delete  wage  types  depending  on  the  initial  wage  type
...
Leave entitlement may either be planned subject to the age or staff membership.

Leave entitlement

The  sum  of  the  fields  Annual  leave,  Special  leave  and  Additional  leave  will  be  credited  to  the

employee's leave account (4th account when defining accounts) on 1st January.

Any  changes  made  to  the  entitlement  to  annual  leave,  special  leave  or  additional  leave  are  only

reflected in the leave account when the labor time calculation for the 1st January is started.

Valid from, to

These two fields restrict the validity of the leave entitlement to a specific period of time. If only one

of these two fields is filled, then the entry is valid from or to that date.
...
Master Data  Labor Time  Attendance Bonus
...
The  “attendance  bonus”  module  allows  for  the  assignment  of  an  additional  allowance  (e.g.  attendance

bonus) for employees who work on special days. It is also possible to define fixed special payments, such

as travel expenses, allowance for food, or similar. Posting is triggered using a wage type. When this wage

type  (initial  wage  type)  occurs,  a  time  bonus  is  posted  onto  the  “presence  wage  type”.  The  initial  wage

type may be deleted, once everything has been settled. The presence wage type is posted subject to the

duration posted on the initial wage type.
...
The additional allowance is granted when this wage type is posted. Several rules may be defined

for an initial wage type. The rule that has the greatest possible “as of duration” is processed for the

initial wage type.

Reference

Attendance  bonuses  can  be  defined  subject  to  the  duration  or  a  percentage  rate  of  the  target  or

normal time.

Duration/percentage

Amount of time that has at least to be posted onto the initial wage type by the payroll day in order

for the posting to the presence wage type to be made.

The required amount of time may be defined as absolute duration or subject to a percentage rate of

the target or normal time.

Delete

All postings of the initial wage type are deleted and are no longer available, once the rule has been

executed.

Presence wage type

Wage type onto which the time bonus from the “duration” field is posted.

Duration

Time posted onto the presence wage type. Values are entered in the hours:minutes format.

Alignment

Before:

The attendance bonus is posted so that it ends at the selected point in time.

After:

The attendance bonus starts at the selected point in time.

Point in time

Point in time when posting to the presence wage type is to take place:

Midnight:

Posting takes place at 0:00 midnights.

Beginning:

Posting is set to the beginning when the initial wage type was posted the first time.

End:

Posting is set to the end when the initial wage type was posted the last time.
...
Posting to the presence wage type is subject to authorization.

Company

The attendance bonus is restricted to a specific company. If no company is entered the attendance

bonus applies to all companies.

Valid from, until

Validity  period  for  the  attendance  bonus.  The  attendance  bonus  applies  without  restriction  if  both
