...
The function package Client Shop Floor Information of the MES-Cockpit provides extensive functions to

display online information on the following objects for the selected instance:
...
The KPI monitor provides  an overview of the entire site. The user can  drill down all key figures

with reference to cost centers, machine groups and the single object "machine".
...
Search for contact persons in the company and check their attendance status.
