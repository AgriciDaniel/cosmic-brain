...
This  function  package  provides  the  opportunity  to  configure  the  access  control  module  so  as  to  trigger

random security checks on employees when they leave the factory premises.

Implementation notes

You use the function package if you:

  use the HYDRA access control system (ZKS) and wish to determine which employees are to be

checked when leaving the factory premises.
...
The exits need to be controlled by the HYDRA access control module to be able to use this function.
...
o  Employee and security personnel are notified of the security check and the entrance can

be blocked (optional)

o  Exception for specific badges that do not require security checks (e.g. management)
