...
In  combination  with  the  access  control  module,  this  function  package  allows  to  record  the  presence  of

employees  in  room  zones,  to  control  elevators  with  different  authorizations  for  each  floor  and  to  map

checkpoints where only one door may be opened at a time in the system.

Implementation notes

You use the function package if you:

  use the HYDRA access control system (ZKS) and wish to monitor room zones, control elevators
...
o  Definition of room zones and by which entrances these room zones are entered or exited

o  Checking of presence or absence when entering or leaving a room zone

o  Room  zone  overview  to  show  the  employees  who  are  currently  present  or  who  were
