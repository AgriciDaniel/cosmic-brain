...
The function  package  can  be  added  on  at  any  time.  In  order  for  it  to function  correctly,  key  information

must  be  correctly  entered  in  HYDRA.  In  addition,  the  access  paths  must  be  correctly  set  at  the

documents to be displayed (so-called path configuration)
