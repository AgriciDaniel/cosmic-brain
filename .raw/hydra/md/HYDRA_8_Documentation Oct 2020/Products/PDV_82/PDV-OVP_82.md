...
The  PDV-OVR  function  package  includes  functions  for  online  visualization  of  data  in  the  MOC  client.

Connections can be established to all PDV data servers available in the HYDRA network accessible from

the  infrastructure.  Data  from  these  servers  is  visualized  online.  The  logical  channels  and  collection

requests currently running on the machines specify which data can be displayed.

Existing  display  instruments  and  a  trend  graphic  are  available  to  visualize  data.  Special  layouts

visualizing  facilities  or  parts  thereof  can  be  integrated  into  the  system  using  suitable  editors.  Thus

creating what is effectively a machine cockpit allowing to view process data of facilities.

Integration

This  function  package  requires  a  machine  interface  for  data  collection  in  the  HYDRA  Process

Communication  Controller  (PCC),  the  integrated  data  server,  the  network  infrastructure  and  the  basic

PDV-PDM package including collection rules.

Features

  Permanent display of process data for one or more machines and facilities. If you have the proper

license, you can generate images of facilities on your own (development license required) or you

can order HYDRA Customizing and have them generated by MPDV (including runtime license for

process visualization).

  Measured  values  are  displayed  online  with  a  predefined  pointer  graphic  for  a  machine.  The

displayed characteristics can be selected interactively.

  Measured  values  are  displayed  online  showing  a  trend  line  for  a  machine.  You  can  select  the

displayed characteristic interactively. The function visualizes the progress and the last measured

values for this characteristic online as a continuous trend graphic.
