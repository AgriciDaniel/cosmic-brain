...
HYDRA Escalation Management provides a framework of functions for prompt forwarding of recorded or

live events to individual users or user groups. Escalation Management actively notifies users.
...
You use Escalation Management if you would like to have active, prompt notification of specific events in

the BDE production environment to react early enough to prevent downtimes.

Integration

The  events/  escalations  already  triggered  in  the  PDV  environment  are  forwarded  to  central  Escalation

Management. This forms the framework for forwarding and following up on triggered events.
...
  Provision  of  various  escalation  messages  for  Process  Data  Collection  such  as  "action  limits

exceeded" or "tolerance limits exceeded".

  Event configuration: configuration of order-related events.

  Forwarding of detected events to the framework of Escalation Management.
