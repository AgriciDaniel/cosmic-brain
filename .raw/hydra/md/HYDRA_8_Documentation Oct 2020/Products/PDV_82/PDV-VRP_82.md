...
The PDV-VRP function package "Processing rules for process data" provides the functions used to define

collection rules and collection processes. The characteristics defined in the basic package are linked with

the access paths by defining collection rules. This activates PDV data collection

Integration

This  function  package  requires  a  machine  interface  for  data  collection  in  the  HYDRA  Process

Communication Controller (PCC) and the basic package PDV-PDM.

Features

  Development of machine-related collection rules managed in specific versions.  Consolidation  of

characteristics planned for this machine. Definition of limits. Specification for statistics processing.

  Development  of  article-related  collection  rules  managed  in  specific  versions.  Consolidation  of

characteristics planned for this article. Definition of limits. Specification for statistics processing.

  Taking the  specification  list into  account,  planned  values can  be  redefined by  referring  to  tools,
...
then used as the basis for collecting measured values.

  Definition of conversion factors for characteristics and the ability to calculate new characteristics

from existing characteristics.

  Collection rules are converted into active collection requests by order processing (for article-related

collection  rules)  or  manual  specification  (machine-related  collection  rule).  Collection  requests

regulate data collection and provide the ability to evaluate the history.

  When the collection request is changed, active collection processes are automatically notified and,

as a result, collection specifications automatically change at runtime.
