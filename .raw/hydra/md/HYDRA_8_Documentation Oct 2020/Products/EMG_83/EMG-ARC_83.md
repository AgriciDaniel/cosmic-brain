...
2  Configurations specific to EMG .................................................................... 5
...
You can use the function package "archiving of energy consumption data" to access archived data and to

export collected data. If required, you can also re-import this data back into the system.

You use the function package for the following purposes:

  You want to access data of this function package that is already moved to archive tables.

  You  want  to  ensure  the  long-term  storage  of  collected  data  due  to  legal  or  customer

requirements.

  You  want  to  import  data  into  the  system  that  has  already  been  exported  and  perform  a  new

evaluation/analysis because of legal or customer requirements.

Integration

The  archiving  function  for  consumption  data  accesses  inventory  data  and  collected  data  in  the  relevant

archive tables of the function package. You can also use the archiving functions of the function packages

Process  Data  and  Resources  Management  to  control  the  performance  data  collected  for  the  Energy
...
o  Direct access to archived shop floor/order data collected by the applications
...
o  Functions to transfer data from online tables to archive tables

  Export function

o  Functions  to  transfer  (export)  data  from  the  archive  tables  to  external  file  systems  with

the objective of a long-term storage of the collected consumption data.



Import function

o  Functions  to  import  exported  data  to  archive  tables  in  order  to  evaluate  them  using  the
...
2  Configurations specific to EMG

Overview

The  product  group  EMG  stores  data  for  35  days  by  default,  before  the  data  is  moved  to  the  long-term

storage. There are different entries with defined retention periods for each of the compression levels.

The EMG archiving function processes the following data:

  Documents

  Compression level "year"

  Compression level "month"

  Compression level "week"

  Compression level "day"

  Compression level "hour"

  Compression level "15 minutes"

Configuration

Using HYDRA Data Management, you can configure how long data is to be kept in individual data areas.

Data exceeding the "retention period" (in number of days/months/years; see the values in parentheses in

the table below) is transferred into archive tables. If the EMG archiving license is not available, the data is

deleted once the configured retention period has expired.
...
Documents compressed to  Online data
...
1)  Please note that by default the system only archives the documents and not the compressed data. If

you want to archive compressed data, you must use the "export" function.

2)  Please  note  that  the  "Use  of  resources"  only  allows  data  to  be  edited  in  the  online  data  area.  The

edited documents are transferred to the compressed data in the course of the next compression. You

cannot directly edit the compressed data.
