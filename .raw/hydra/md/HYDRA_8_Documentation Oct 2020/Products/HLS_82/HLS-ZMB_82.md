...
1  Overview: Objective-Driven Machine Assignment ....................................... 4
...
1
 ...
 Overview: Objective-Driven Machine Assignment

Purpose

Priority  rules  are  used  in  order  processing  for  detailed  planning  (and  sequencing),  once  an  order  has

been released. With the aid of priority rules, detailed planning specifies how released orders are assigned

to  workstations.  This  becomes  particularly  important  when  several  orders  are  lined  up  at  a  workstation
...
Using  this  priority  rule,  the  operations  to  be  planned  are  rated  and  indexed  using  adjustable  weighting,

and  the  queue  is  sorted  (prioritized)  based  on  this  index.  Unlike  other  sorting  rules,  with  this  one  the

actual objective is rated and the assignment is based on the results.

You  use  the  function  package  if  you  would  like  the  queue  sequence  to  be  formed  based  on  weighted

objectives.

Integration

Planning strategies are used in automatic planning / assignment in order to define a sequence relating to

the next highest-priority orders / operations.

Features

  Planning strategy controlling automatic machine assignment based on weighted objectives:

  Evaluation  of  the  operations  to  be  planned  using  adjustable  weighting  and  generating  an  index,

based on which the queue is sorted

  Configuration  option  defining  the  objective-driven  rule  by  selecting  the  key  performance  indicators

integrated in the rule

  Selection of objective-driven rules when defining planning options
