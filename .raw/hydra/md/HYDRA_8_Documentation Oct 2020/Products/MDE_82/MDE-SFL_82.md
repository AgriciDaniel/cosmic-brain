...
You use the function package if you wish to connect a simple production line covering only one operation.
...
Configuration possibility for aggregates/production units and their assignment to lines

  Line and aggregate monitoring

Monitoring of the line and its aggregates/production units via machine interfaces and automatic

posting of recorded statuses.
...
Special icon view at Windows line terminals
...
Assembly  line  production  involves  the  production  of  large  quantities  of  similar  goods  over  a  specified

period and as part of a specific workflow. The required machines (also called aggregates or workstations)

are arranged according to the sequence of operations. Usually, a conveyor belt automatically moves the

parts/items  from  one  aggregate  to  the  next.  In  general,  a  production  line  consists  of  machines,

aggregates or workstations.

Purpose

Define the machines, aggregates, workstations and the total assembly line as HYDRA machines. The ID

machine  type  in  the  configuration  of  machines/workplaces  distinguishes  these  HYDRA  machines  from
...
Machine/workplace/workstation

Use the line assignment to assign machines, aggregates and workstations to a production line.
...
Unique ID of the production line. Defined in the system as workplace type "L".

Position

Specify the position of the machine, aggregate, workstation in the production line.
...
Specify the machine, aggregate, workstation assigned to the production line.

Reference aggregate

The option reference aggregate specifies if the assigned aggregate is a reference aggregate. You

can only define one reference aggregate for each production line.

Workplace master data

Short name

Short name of a machine, aggregate, workstation (workplace/machine).

Name

Long name of a machine, aggregate, workstation (workplace/machine).

Group

Group the machine, aggregate, workstation (workplace/machine) belongs to.

Cost center

Cost center the machine, aggregate, workstation (workplace/machine) belongs to.
