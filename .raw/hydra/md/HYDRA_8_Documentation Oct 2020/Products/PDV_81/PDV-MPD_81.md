...
Monitoring of Process Data (MOC)
...
1
 ...
 Monitoring Prozessdaten

Summary

Possible fields of application

The PDV-MPD function package includes the functions to analyze process data. The values and events

collected  from  the  machines  may  be  listed  and  modifications,  deviations  from  default  values  and  limit

values that have not been respected can be analyzed.

Integration

This function package requires data to be available in the system (collected by the PDV input functions).

The licenses and functions for shop floor data collection or batch data collection are required to be able to

evaluate the recorded data on the basis of orders and batches.

Functions

  Evaluation of the collection rule values changed by the specification list

  Evaluation of process disturbances for single machines over selectable periods
...
  Process  log  for  individual  machines  or  articles/items  over  selectable  periods.  The  recorded

process parameters can be filtered.

  Tabular listing of the occurred process events. Events and periods can be selected.

  Tabular  listing  of  durations  of  the  occurred  events.  The  durations  are  calculated  by  intervals

considering the beginning and end of events. Events and periods can be selected. The occurred

events  can  be  analyzed  on  the  basis  of  time  and  quantity  using  the  grouping  and  sorting
