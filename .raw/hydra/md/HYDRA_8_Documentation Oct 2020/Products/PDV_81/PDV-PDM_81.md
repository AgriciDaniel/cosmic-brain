...
This  function  packet  contains  the  user  interface  used  to  define  the  necessary  master  data  for  process

data collection. It also contains the service for storing the measured values collected and for generating

samples  for  the  statistical  process  data  reports.  Process  data  collection  can  be  set  up  to  collect  TAG-

oriented entry processes for specific machines. This, among other things, is a prerequisite for ID tracing

(PDV-PTR)

The PDV-VRP function packet "Processing rules for process data" is required to control measured values
...
Extensive configuration functions for creating and editing master data for process values.

  Configuration of logical channels for the entry of process values

  Definition of input and output channels.

  Extensive functions for creating and editing machine and article-related monitoring parameters for
...
  List  for  managing  specific  monitoring  parameters  depending  on  certain  machines,  articles  and

tools

  Lists for defining the binary process events.

  Sample  generation  at  specified  sampling  intervals.  The  intervals  are  defined  when  defining

collection rules characteristics.

  Based on these rules, the service generates sample data that are available for expanded sample
...
  Collection  of  text  tags  for  identifying  the  measured  values.  Optimized  storage  of  related

measured values. The system determines their relationship and saves it to the identification tags

irrespective of the collection time.

  Service for indexing measured values according to the defined identification tags
