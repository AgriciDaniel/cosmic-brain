...
Extended Control Charts and Histograms
...
1  Extended Control Charts and Histograms ... Error! Bookmark not defined.
...
1  Extended Control Charts and Histograms

Purpose

This component is used when the collected inspection data from Production or Outgoing Goods are to be

represented  graphically  as  attributive  and  variable  characteristics  in  the  form  of  control  chart  and

histogram combinations spanning all orders.

Implementation Notes

Use of this component is expedient in particular with short orders with a small number of inspections, as

meaningful statements on the quality situation are only possible above a given data volume. Furthermore,

use is to be recommended when value developments are to be tracked over a prolonged period, e.g. to

monitor the effectiveness of initiated improvement measures.

This  component  can  be  expediently  used  particularly  in  Outgoing  Goods  where  there  is  generally  only

one sample per outgoing goods inspection.

The  basis  for  evaluation  using  this  component  is  that  the  inspection  data  were  collected  in  HYDRA

beforehand.

Integration

The  component  "Inspection  planning  for  in-process  inspections"  with  the  corresponding  inspection

requirements  and  inspection  results  form  the  basis,  whereby  the  inspection  data must  first  be  collected

using the AIP shop floor client.

Features

Inspected characteristics can be visualized graphically spanning all orders in the form of different control

charts,  a  histogram  and  the  list  of  the  data  basis.  The  data  basis  can  thereby  be  restricted  using  a
