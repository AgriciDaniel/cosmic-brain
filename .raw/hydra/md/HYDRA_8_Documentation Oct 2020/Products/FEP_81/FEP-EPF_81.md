...
1  Overview of Expanded Inspection Planning/ Inspection StepsError! Bookmark not defined.
...
1  Expanded Inspection Planning/ Inspection Steps

Purpose

This component expands the inspection planning functions by adding the ability to

  assign gages,



input  of  calculation  formulas  with  which  to  calculate  measured  values  from  the  results  of  other

characteristics

  define results that will trigger inspections

  print out inspection plans.

Implementation considerations

Use  of  this  component  is  recommended  when  events  are  in  use,  such  as  machine  status  change  or

reaching a defined number of produced pieces to trigger inspection due dates.

The  same  applies  to  automatic  measured  value  input  per  interface  device,  because  this  is  required  to

assign gages.

Integration

This component primarily controls the "Inspection planning for production report inspections" component.

Features

The following functions are available:

  The  ability  to  specify  formulas  used  to  automatically  calculate  measurement  results  while

referencing measured values of previous inspection characteristics that have already been input.

  The ability to assign gages/ gage groups that allow the use of automatic measured value entry by

the HYDRA measured data interface.

  The  ability  to  define  events  that  will  trigger  inspection  due  dates,  for  example  after  reaching  a

time/ piece interval, a machine status, an output batch or a shift change

  The option to print out inspection plans, inspection results, etc. in Word format with the capability

of  preparing  your  own  Word  templates  provided  a  license  allowing  Word  template  preparation
