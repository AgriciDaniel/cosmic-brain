...
Alpha-numeric key for the category
...
The  application  "work  equipment  management"  enables  to  manage  the  distribution  and  return  of  work
...
Assigns the entry to a category. Authorizations for accessing work equipment can be controlled by
...
Storage location of the work equipment, provided it is not lent out at the moment
...
A  file  can  be  stored  for  each  entry  included  in  the  "work  equipment  management".  Three  toolbar

buttons allow for a file to be added, viewed or deleted. This field shows the unique file name under

which the file has been stored on the server. The names of the files assigned to  work equipment

start with "weqi".

Handed out on

The point in time of the distribution is automatically assigned the current date when the data record

is created. But this can still be changed manually. The "handing out" toolbar function assigns this

field with the current point in time.

Handed out by

The user name of the person handing out the equipment is set automatically if a personnel number

is entered and the handout date is changed. The field cannot be changed manually.

Returned on

The return date and time can be entered manually. This field can be assigned the current date by

clicking the "return" function of the toolbar.
...
User name of the person receiving/accepting the equipment. Is set automatically if the return time is

changed. If the field for the return date is empty, the data entered in the "received by" field will be

deleted. The field cannot be changed manually.

Modified by, modified on

Editor as well as date and time the last modification was made.
...
displayed by the relevant application connected in the operating system.
...
By  clicking  this  function,  the  "handed  out  on"  field  is  assigned  the  current  point  in  time.  If  the

personnel  number  is  not  entered  as  part  of  the  hand  out  process,  an  error  message  will  be

displayed and the handout is not carried out.

 Return

Using this function, the "returned on" field is assigned with the current point in time.
