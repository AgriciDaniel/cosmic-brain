...
When starting MOC, the user name and password will be requested. HYDRA offers the possibility to assign
...
areas  and  possibly  for  cost  centers.  This  is  also  true  for  evaluations/reports  and  information  dialogs

displaying "confidential" data.

Prior to be allowed to work in MOC, the following activities must be performed for each user.

-  Creation of the user

-  Assignment of function authorizations

-  Assignment of cost center authorizations

-  Assignment of responsibility areas
...
The selection panel can be used to filter by superordinate or by assigned resources. The following selection

criteria are available in the application:
...
A unique/unambiguous MOC user identification must be entered here. We recommend using already

applied user names from mail programs or ERP programs to ensure that the person concerned can

use the same user name in all programs.

Name

The name describes the user more precisely. Enter the first name and name here.
...
These fields have been designed to assign the user to a person within the HR master.
...
- These fields are only shown if the Single Sign On license is available.

- In combination with the following INI entry, the fields to enter the password / requesting to change
...
Copy function: The SSO configuration of the user to be copied will not be copied.
