...
1  Resources Management - Archiving ............................................................ 4

2  WRM-Specific Configurations ...................................................................... 5

3  Data Management ........................................................................................ 7

4  Functions/configurations specific to product groups .................................. 10
...
1
 ...
 Resources Management - Archiving

Overview

Purpose

Resource information archiving refers to the resource data that is generated as a result of managing objects

and orders. As with the other MES areas, archiving is divided into three parts:

-  Active  data  set:  Current  data  accumulate  here  and  can  also  be  edited  here  using  the  editing

functions.

-  Medium-term archive data  set: The data are  in  a different area  within the database and can no

longer be modified, but are completely available in the reports.

-  Backed-up long-term data area. The data are no longer available in the reports; they have been

backed up onto data media outside the database. The data can be restored into the database for

evaluation purposes.

Features

Configuration functions and administrative operations used to control how resource data is archived.

  Archiving function to automatically transfer collected resource data (postings, events) into archive

tables.

  Direct  access  to  the  archived  resource  data  from  the  functions  resource  history  and  use  of

resources at the HYDRA console

  Ability to export data from the archive tables onto external file systems in HYDRA standard format

(using the customer's server-based unload/ load function)

  Ability to import backed up data into the archive tables so as to be able to evaluate them at the

HYDRA console using the resource history and use of resources functions (using the customer's

server-based unload/load function)

  Administration of archive data throughout preset periods of time in the HYDRA database.
...
2  WRM-Specific Configurations

Overview

You  can  archive  the  following  data  according  to  project-specific,  internal  or  even  legal  specifications  to

access this data at a later point in time.
...
WRM archiving includes the following data:

  Log records (tables res_belege, a_res_belege)

  Events (tables event_res, a_event_res)

  Additional information (tables hyd_logging, a_hyd_logging_data)

  Document links (tables hyd_document, a_hyd_document)

Configuration

Use the HYDRA Data Management to configure how long data is kept in the different data areas.

Data  is only  transferred to  the archive tables  when the "retention  period" has expired. For implemented

archiving processes, the default values are specified. You can configure different values for each system.

If the WRM archiving license is not available, the data is deleted once the configured retention period has
...
Configuring the archiving of the document management
...
As part of the HYDRA document management of the WRM, you can assign documents to the following

objects.

  Resources

  Maintenances of the activity calendar

  When resetting the maintenances performed

To archive the document entries, make the following entries in the HYDRA data management.
...
Moving the document entry from the online data to the

0 days

medium-term data stock.

WRM

A_DOCLINK  Moving the document entry from the medium-term data
...
controls the action. This condition is linked to the other conditions of the original command using AND

and is set in parentheses.
...
data is archived for each object individually). Z = Time-related archiving (i.e. data is archived without
...
Unique key column in the master table; this key identifies the data to be archived. You can define up

to 5 key columns for object-related archiving. Time-related archiving only supports one primary key
...
4  Functions/configurations specific to product groups
...
The standard archiving function of HYDRA-PDV 7.2 can archive mass data tables. The system writes table

data into files that are stored using a defined path.
...
information on the location and the name of the data file, or on an eventual re-import of the table that might
...
The export is performed at cyclic intervals and is started by the system scheduler. The export unloads “due”

tables from the database into data files on hard disk.
...
also configure a one-to-many relationship, i.e. you can distribute a table among several files. Vice versa, a

file must belong to exactly one table (one table can have several files, but a file always refers to exactly
...
As part of a customization, you can also archive further data ( e.g. events). To this end, you must store the

respective entries in the table "hyd_datamanagement". The process is identical to the one of the general
...
For information on the data management configurations of the Escalation Management in version 3.0, refer

to the document MBL_ESK_Archiving.pdf.
...
backed up it is transferred from the long-term data area to the archive data area and, as a result, it is stored

in a separate file system. The customer can start the backup process using the Reload Manager.
...
button). Once the function has been started, the input dialog that opens requires the customer archive
...
specify another storage location for the files, provided that the archived files have not been moved to

the customer-specific archive using the Reload Manager.
...
In order for data to be transferred to the reload data area, the HYDRA server must be able to access

these files. Otherwise, loading is cancelled with an error message.
