...
The  resource  schedule  shows  the  planned  use  of  resources  in  a  Gantt  chart  for  primary  resources

(workplaces, machines) and secondary resources (e.g. tools). It also shows the point in time and duration

of the assignment. In addition, the application provides a tabular overview for the provision of tools (list of
...
to analyze and visualize resource allocation over time.

to optimize the provision of tools using the list of required resources.
...
This application does not provide for changes to the assignment. Resource allocation

may  only  be  modified  by  planning  operations  using  the  graphic  planning  function  and/or  by

replanning in the ERP system.

Functions

Dynamic  management  of  a  resource  schedule  based  on  planned  production  operations  and  the

assignment of resources to operations:

  Observation  of  resource  statuses  and  maintenance  orders  for  resources  (provided  the  relevant

functions are used in the HYDRA-BDE module)

  graphic evaluation of the resource schedule



tabular listing of planned dates for the use of resources (resource requirements list)
