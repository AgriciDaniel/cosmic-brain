...
1  Configuration E-Report Manager
...
Assign a file name for the report without a file extension (*.lul).

The  report  must  be  stored  and  exported  on  the  server  in  order  to  use  the  report  in  the  E-Report

Manager.  You can find details in the chapter „Error! Reference source not found.“.
...
Depending on the configuration of the paths on the server, the report file is located in the configured

path MOCREP (e.g. \\MyServer\HYDRA3\3\custom\reports\MyTestReport.lul).

Application ID

Application the report belongs to.

You need this information to configure the required selection parameter.

User

User that runs the report. The system checks the responsibility area the HYDRA user is authorized

for. The function authorization for the user have no effect in the E-Report Manager.
...
Further formats according to the output options provided in List & Label.

PDF, BMP, HTML, JPG, PNG, XPS, XLS, XLSX and XML. The display may differ for each output

format. You must bear in mind and test the output format when you design a report.  Common output

formats are PDF and perhaps HTML.
...
Printer name to print the generated file afterwards. The printer name refers to a printer io the server

where the E-Report service has been installed.

To use a printer, ensure that the Windows user  configured for the E-Report service can access a

printer and has the authorizations. The system administrator knows the available printers. The system

administrator installs the printers according to the installation instructions "SIS-ERM_30_installation"

in  the  E-Report  server.  The  document  "SIS-ERM_30_installation"  also  describes  how  to  test  the
...
All fields are connected by a logical AND relation. If fields are not populated, the value is not used

and the action is executed again and again.
...
If you want to run a report and output the data in a relative manner (e.g. using the data of the previous

day), you must configure the field in the selection panel of the dialog as relative date field.
...
If the e-mail is not dispatched during the test, disable the e-mail dispatch in the E-Report configuration

in the  Escalation tab before running the test.
...
In order to use reports for the E-Report Manager, the reports must first stored on the server and then be

exported to the MOC with the MES Development Suite activated.

1.  Activate the „MES Development Suite“.

2.  Open your report in the application that includes your report - ReportConfiguration.

3.  Select the required report and call the function Edit.

4.  Activate the option "Save on server".
...
5.  Execute the "Export" function.

6.  Close the ReportConfiguration with OK.

The report is stored on the server in the path "MOCREP".  The export files are stored in the HYDRA path

"PSLLXML" and are then available to the E-Report Manager.
...
We use the report Maintenance plan of the application Activity calendar as an example. You must first of

all enable the MES Development Suite to export the report and then open the activity calendar. You can

use the transaction code "rmcal" to open the Activity calendar.
...
Then the dialog ReportConfiguration opens.

1.  Select the report and call the "Edit" function.

2.  Activate the option "Save on server".

3.  Execute the "Export" function.

4.  Close the ReportConfiguration with OK.

A prompt confirms the successful export.

The export of the report configuration is stored in the HYDRA server according to the path configuration

PSLLXML. In the example, the following file is saved after the export using the configured path PSLLXML:
...
Depending on the configuration in tab General, you only need to configure tab Fixed point in time or Interval.

To configure the e-mail dispatch, use tab Escalation.
...
can occur. Malfunctions can affect the export of reports, the E-Report configuration and the test of reports,

if the settings and configurations made during the installation are not correct or not correct any more.
...
  The HYDRA paths PSREPORT and PSLLXML are created, if not yet available. The path MOCREP

is checked. These paths are required to export report files, to use the files in the E-Report manager

and to save the created reports.
...
  An INI configuration including the address of the E-Report server is created. This INI configuration

is required to test reports. Using this address, the print server is triggered to immediately create the
...
If problems occur during configuration, test or application of the E-Report manager, check all configuration

steps included in the installation instructions "SIS-ERM_30_installation".
